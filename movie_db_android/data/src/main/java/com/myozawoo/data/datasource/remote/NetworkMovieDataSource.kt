...
class NetworkMovieDataSource{
    ...
    fun getMovieDetails(movieId: Int): Observable<MovieDetailsResponse> {
        return movieService.getMovieDetails(movieId = movieId, apiKey = BuildConfig.MOVIE_DB_KEY)
            .map {
                try {
                    if(it.isSuccessful) it.body()
                    else {
                        val errorResponse = Gson().fromJson(it.errorBody()?.charStream(), MovieDetailsResponse::class.java)
                        throw ApiException(errorResponse.status_message ?: "Something went wrong!")
                    }
                }catch (e: IOException){
                    throw NetworkException
                }
