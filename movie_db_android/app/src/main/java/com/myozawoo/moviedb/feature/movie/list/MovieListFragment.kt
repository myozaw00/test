...
import com.myozawoo.moviedb.feature.movie.details.MovieDetailDialogFragment
...
class MovieListFragment: BaseFragment<MovieListViewState, MovieListView, MovieListPresenter>(),
    MovieListView,
    MovieListRecyclerAdapter.MovieClickListener{
    ...
    private fun setup(){
        mMovieAdapter = MovieListRecyclerAdapter(this)
    ...
    override fun onMovieClick(info: MovieInfo) {
        val optionDialog = MovieDetailDialogFragment(info.id)
        val ft = childFragmentManager.beginTransaction()
        val prev =
            childFragmentManager.findFragmentByTag("movie_details")
        if (prev != null) {
            ft.remove(prev)
        }
        ft.addToBackStack(null)
        optionDialog.isCancelable = true
        optionDialog.show(ft, "movie_details")
    }
